defmodule RiverSideWeb.AdminLive.Dashboard do
  @moduledoc """
  LiveView module for the admin dashboard interface.

  This module provides comprehensive administrative capabilities for managing
  the River Side Food Court system. It serves as the central control panel
  for system administrators to manage users, vendors, and system settings.

  ## Features

  ### User Management
  * View all users with role-based filtering
  * Create new users (customers, vendors, cashiers, admins)
  * Edit existing user details and roles
  * Delete users with cascade cleanup
  * Real-time user statistics

  ### Vendor Management
  * Create vendor accounts with associated users
  * Monitor vendor activity and status
  * Handle vendor deletion with proper cleanup

  ### System Operations
  * Reset all tables to available status
  * View system statistics and metrics
  * Monitor active sessions and usage

  ## Security

  * Requires admin role for access (enforced by router)
  ...
  * New users are created or modified
  * System statistics change
  * Tables are reset or updated

  ## Modal Interfaces

  The dashboard uses modal dialogs for:
  * Creating new users
  ...
  * User management table with filtering and actions
  * Modal dialogs for create/edit/delete operations

  Uses DaisyUI components for consistent styling and responsive design.
  """
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              ...
              <div class="card bg-base-200">
                <div class="card-body p-3 sm:p-4">
                  ...
                  <button
                    phx-click="reset_all_orders"
                    data-confirm="Are you sure you want to delete ALL orders? This action cannot be undone."
              ...
              <div class="card bg-base-200">
                <div class="card-body p-3 sm:p-4">
                  ...
                  <a href={~p"/admin/reports"} class="btn btn-primary btn-sm">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
  ...
  @impl true
  @doc """
  Initializes the admin dashboard LiveView.

  Sets up the initial state with:
  * List of all users from the database
  * User statistics by role
  * Form changesets for create/edit operations
  * Modal visibility flags

  ## Parameters

  * `_params` - Unused URL parameters
  * `_session` - Unused session data
  * `socket` - LiveView socket
  ...
  def handle_event("reset_all_orders", _params, socket) do
    # Delete all orders and order items
    Repo.delete_all(OrderItem)
    Repo.delete_all(Order)

    # Log the action
    Logger.info("Admin reset all orders - deleted all orders and order items")

    {:noreply,
     socket
     |> put_flash(:info, "All orders have been deleted successfully.")}
  end
